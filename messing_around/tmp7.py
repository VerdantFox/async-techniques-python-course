...
async def download_some_async(show_num: int = 174) -> int:
    ...
    print(f"-------------Async thread: {current_thread().name}", flush=True)
    print(f"-------------Async loop id: {id(asyncio.get_event_loop())}", flush=True)
...
def download_some_threaded(show_num: int = 174) -> int:
    ...
    resp.raise_for_status()
    print(f"Downloaded (more) {len(resp.text):,} characters.", flush=True)
    print(f"------------sync thread: {current_thread().name}", flush=True)
    return resp.status_code
